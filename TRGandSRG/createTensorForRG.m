function createTensorForRG(parameter, waveLeft, waveRight)

siteDim = parameter.siteDimension ;

systemSuffix = parameter.systemSuffix ;

LambdaLeft = waveLeft.Lambda ;
LambdaRight = waveRight.Lambda ;
%* A(x,y,z,m) = sqrt(LambdaX(x)*LambdaY(y)*LambdaZ(z))*A0(x,y,z,m)
%* B(x,y,z,m) = sqrt(LambdaX(x)*LambdaY(y)*LambdaZ(z))*B0(x,y,z,m)
[Aleft, Bleft] = absorbLambda(siteDim, waveLeft) ;
[Aright, Bright] = absorbLambda(siteDim, waveRight) ;
clear waveLeft waveRight
%* TA((x1,x2),(y1,y2),(z1,z2))=sum{m}_(A(x1,y1,z1,m)*A(x2,y2,z2,m))
%* TA((x1,x2),(y1,y2),(z1,z2))~TA(x,y,z)
As(1).AA = createAA_imba(siteDim, Aleft, LambdaLeft, Aright, LambdaRight) ;
clear A
TA(1) = createTA(As(1).AA) ;
TA(1) = reduceTA(TA(1)) ;


Bs(1).BB = createAA_imba(siteDim, Bleft, LambdaLeft, Bright, LambdaRight) ;
clear B
